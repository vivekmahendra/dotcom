...
export async function handleNewsletterAction(request: Request) {
  ...
  try {
    ...
    if (reset === "ratelimits" || email === "reset@ratelimits") {
      resetRateLimits();
      return Response.json({
        success: true,
        message: "Rate limits have been reset. You can now subscribe again."
      });
    }
    ...
    if (error instanceof Error) {
      ...
      if (error.message === "Email already subscribed") {
        // Return success with a different message for already subscribed users
        return Response.json({
          success: true,
          message: "You're already subscribed! Thanks for your continued interest. You'll receive updates when I publish new content.",

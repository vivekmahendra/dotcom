    ...
    <section className="py-16">
      <div className="mb-8">
        <div className="flex items-center gap-4 mb-2">
          <h2 className="text-3xl font-light">{TEXT_CONFIG.home.sections.ideas.title}</h2>
          <Link 
            to="/ideas"
            className="group px-4 py-1 text-sm border border-gray-300 rounded-full hover:bg-gray-50 transition-colors"
          >
            {TEXT_CONFIG.home.sections.ideas.viewAllText} <span className="inline-block transition-transform group-hover:translate-x-1">→</span>
          </Link>
        </div>
        <p className="text-gray-600">{TEXT_CONFIG.home.sections.ideas.subtitle}</p>
      </div>
        ...
        {ideas.slice(0, 3).map((post, index) => (
          <div key={post.slug}>
            <article className="py-6">
              ...
              >
                {TEXT_CONFIG.ui.buttons.readMore} <span className="inline-block transition-transform group-hover:translate-x-1">→</span>
              </Link>
            </article>
